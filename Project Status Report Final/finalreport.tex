...
%	\thanks{This work was contributed by }
	...
	Fig \ref{uniquevID} assists in describing how the total number of stocks owned has two values. Here, the levels at which unique stocks are owned in inverted compared to Fig \ref{totalvID}. Both risk adverse and risky Brokers tend to own a number of unique stocks while those with medium risk tend to own less of the same type of stocks. The ownership of multiple stocks in different industries can be seen as less risky as owning more individual stocks allows certian investments to fail without affecting the total as much. This is compared to a high ownership of a specific stock, which if goes under, results in the entire portfolio going under as well.  
