...
class Stock:
    def __init__(self, ticker, forward_eps, earnings_growth, dividend_ratio, dates, prices):
        ...
        while current_date < datetime.strptime("12/31/2023", "%m/%d/%Y"):
            if current_date not in self.date_to_price:
                previous_date = current_date - timedelta(days=1)
                while previous_date not in self.date_to_price and previous_date >= datetime.strptime(dates[0], "%m/%d/%Y"):
                    previous_date -= timedelta(days=1)
                if previous_date in self.date_to_price:
                    self.date_to_price[current_date] = self.date_to_price[previous_date]
